...
def validate_schedule(
    ...
    for d in range(num_days):
        for shift in SHIFT_IDS:
            ...
            assigned_agents = [
                aid
                for aid in scheduling_input.agent_ids
                if schedule.get(aid, [""] * num_days)[d] == shift_key
            ]

            senior_count = sum(
                1 for a in assigned_agents if a in scheduling_input.senior_ids
            )
            junior_count = sum(
                1 for a in assigned_agents if a in scheduling_input.junior_ids
            )
            english_count = sum(
                1 for a in assigned_agents if a in scheduling_input.english_ids
            )
    ...
    for agent_id in scheduling_input.agent_ids:
        assignments = schedule.get(agent_id, [])
        for d in range(num_days - 1):
            if assignments[d].startswith("shift_") and assignments[d + 1].startswith(
                "shift_"
            ):
