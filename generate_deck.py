...
def add_metric_card(slide, left, top, width, height, label, value, 
                    bg_color=WHITE, value_color=CYAN, label_color=MEDIUM_GRAY):
    ...
    add_textbox(slide, left + Inches(0.2), top + Inches(0.7),
                ...
                label, font_size=11, color=label_color, bold=False,
                alignment=PP_ALIGN.CENTER)
    return card
...
def add_section_badge(slide, left, top, text, bg_color=CYAN):
    ...
    shape = add_shape(slide, left, top, Inches(1.8), Inches(0.35), bg_color, corner_radius=0.15)
...
def add_footer(slide, text="Aura Bangkok Clinic  |  CS Demand Forecasting & Shift Scheduling"):
...
def make_slide_header(slide, title, subtitle=None):
    ...
    add_textbox(slide, Inches(0.8), Inches(0.4), Inches(11), Inches(0.7),
                title, font_size=32, color=DARK, bold=True)
    if subtitle:
...
def slide_title(prs):
    ...
    add_textbox(slide, Inches(1), Inches(3.4), Inches(11), Inches(0.5),
                ...
                font_size=20, color=WHITE, bold=False, alignment=PP_ALIGN.LEFT)
    ...
    # Author info
    add_textbox(slide, Inches(1), Inches(4.4), Inches(11), Inches(0.4),
    ...
    add_textbox(slide, Inches(1), Inches(5.8), Inches(11), Inches(0.4),
                "Aura Bangkok Clinic",
                font_size=14, color=WHITE, bold=True, alignment=PP_ALIGN.LEFT)
...
def slide_problem(prs):
    ...
    for i, (title, desc, color) in enumerate(cards):
        ...
        card = add_shape(slide, left, Inches(1.8), Inches(2.8), Inches(2.2), color, 0.05)
        add_textbox(slide, left + Inches(0.2), Inches(2.0), Inches(2.4), Inches(0.5),
                    title, font_size=22, color=WHITE, bold=True, alignment=PP_ALIGN.CENTER)
        add_textbox(slide, left + Inches(0.2), Inches(2.6), Inches(2.4), Inches(1.0),
                    desc, font_size=14, color=WHITE, alignment=PP_ALIGN.CENTER)

    # Constraints summary
    add_textbox(slide, Inches(0.8), Inches(4.4), Inches(11), Inches(0.4),
                "Business Constraints", font_size=20, color=DARK, bold=True)
    ...
    add_bullet_list(slide, Inches(0.8), Inches(4.9), Inches(11.5), Inches(2.0),
                    constraints, font_size=14, color=DARK)


def slide_approach(prs):
    """Slide 3: Solution approach overview."""
    slide = prs.slides.add_slide(prs.slide_layouts[6])
    make_slide_header(slide, "Solution Architecture", "Two-stage pipeline: Predict → Optimize → Validate")

    # Stage boxes
    stages = [
        ("1", "FORECAST", "Predict ticket volume\nper shift per day", "GradientBoosting\nRegressor", CYAN),
        ("2", "STAFFING", "Determine minimum\nSr/Jr/Eng per shift", "Quality Models\n+ Grid Search", DEEP_BLUE),
        ("3", "SCHEDULE", "Assign 50 agents\nto shifts × 30 days", "Google OR-Tools\nCP-SAT Solver", CYAN),
        ("4", "EVALUATE", "Validate constraints\n& project quality", "Fairness + CSAT\n+ Remediation", DEEP_BLUE),
    ]
    ...
    for i, (num, title, desc, tech, color) in enumerate(stages):
        ...
        # Step number
        add_textbox(slide, left + Inches(0.2), Inches(1.9), Inches(2.4), Inches(0.5),
                    f"Stage {num}", font_size=12, color=WHITE, bold=True, alignment=PP_ALIGN.CENTER)
        # Title
        add_textbox(slide, left + Inches(0.2), Inches(2.3), Inches(2.4), Inches(0.5),
                    title, font_size=20, color=WHITE, bold=True, alignment=PP_ALIGN.CENTER)
        # Description
        add_textbox(slide, left + Inches(0.2), Inches(2.9), Inches(2.4), Inches(0.8),
                    desc, font_size=13, color=WHITE, alignment=PP_ALIGN.CENTER)
        # Tech badge
        tech_box = add_shape(slide, left + Inches(0.3), Inches(3.9), Inches(2.2), Inches(0.7),
                             WHITE, 0.08)
        add_textbox(slide, left + Inches(0.3), Inches(3.95), Inches(2.2), Inches(0.6),
                    tech, font_size=11, color=color, bold=True, alignment=PP_ALIGN.CENTER)

        # Arrow between stages
        if i < 3:
            arrow_left = left + Inches(2.85)
            add_textbox(slide, arrow_left, Inches(3.0), Inches(0.3), Inches(0.5),
                        "→", font_size=28, color=MEDIUM_GRAY, bold=True, alignment=PP_ALIGN.CENTER)
...
def slide_forecasting(prs):
    ...
    slide = prs.slides.add_slide(prs.slide_layouts[6])
    make_slide_header(slide, "Task A: Demand Forecasting", "Two-stage approach — Volume Prediction → Staffing Optimization")

    # Left column: Volume Model
    add_section_badge(slide, Inches(0.8), Inches(1.6), "STAGE 1: VOLUME")
    add_textbox(slide, Inches(0.8), Inches(2.1), Inches(5.5), Inches(0.4),
                "GradientBoostingRegressor", font_size=18, color=DARK, bold=True)
    ...
    add_bullet_list(slide, Inches(0.8), Inches(2.5), Inches(5.5), Inches(2.0),
                    vol_details, font_size=13)

    # Volume metrics
    add_metric_card(slide, Inches(0.8), Inches(4.4), Inches(2.5), Inches(1.1),
                    "Validation R²", "0.927", value_color=SUCCESS)
    add_metric_card(slide, Inches(3.6), Inches(4.4), Inches(2.5), Inches(1.1),
                    "Validation MAE", "19.7 tickets", value_color=CYAN)

    # Right column: Staffing Model
    add_section_badge(slide, Inches(7), Inches(1.6), "STAGE 2: STAFFING")
    add_textbox(slide, Inches(7), Inches(2.1), Inches(5.5), Inches(0.4),
                "Quality Models + Grid Search", font_size=18, color=DARK, bold=True)
    ...
    add_bullet_list(slide, Inches(7), Inches(2.5), Inches(5.5), Inches(2.4),
                    staff_details, font_size=13)

    # Quality metrics
    add_metric_card(slide, Inches(7), Inches(4.4), Inches(2.5), Inches(1.1),
                    "CSAT Model R²", "0.818", value_color=SUCCESS)
    add_metric_card(slide, Inches(9.8), Inches(4.4), Inches(2.5), Inches(1.1),
                    "Wait Model R²", "0.963", value_color=SUCCESS)

    # Why GBR callout
    add_shape(slide, Inches(0.8), Inches(5.8), Inches(11.7), Inches(1.0), LIGHT_GRAY, 0.03)
...
def slide_scheduling(prs):
    ...
    slide = prs.slides.add_slide(prs.slide_layouts[6])
    make_slide_header(slide, "Task B: Shift Scheduling Engine", "Constraint Programming with Google OR-Tools CP-SAT")

    # Left: Hard Constraints
    add_shape(slide, Inches(0.6), Inches(1.7), Inches(5.8), Inches(4.4), LIGHT_GRAY, 0.03)
    add_textbox(slide, Inches(0.8), Inches(1.8), Inches(5.4), Inches(0.4),
                "Hard Constraints (must satisfy)", font_size=18, color=ACCENT_RED, bold=True)
    ...
    add_bullet_list(slide, Inches(0.8), Inches(2.3), Inches(5.4), Inches(3.5),
                    hard, font_size=13, color=DARK)

    # Right: Soft Constraints
    add_shape(slide, Inches(6.8), Inches(1.7), Inches(5.8), Inches(4.4), LIGHT_GRAY, 0.03)
    add_textbox(slide, Inches(7.0), Inches(1.8), Inches(5.4), Inches(0.4),
                "Soft Constraints (optimize)", font_size=18, color=AMBER, bold=True)
    ...
    add_bullet_list(slide, Inches(7.0), Inches(2.3), Inches(5.4), Inches(2.0),
                    soft, font_size=13, color=DARK)

    # Model size
    add_textbox(slide, Inches(7.0), Inches(3.8), Inches(5.4), Inches(0.4),
                "Model Complexity", font_size=16, color=DARK, bold=True)
    model_stats = [
    ...
    add_bullet_list(slide, Inches(7.0), Inches(4.2), Inches(5.4), Inches(1.5),
                    model_stats, font_size=13, color=DARK)

    # Why CP-SAT callout
    add_shape(slide, Inches(0.6), Inches(6.3), Inches(12.0), Inches(0.8), CYAN, 0.03)
    add_textbox(slide, Inches(0.8), Inches(6.35), Inches(11.6), Inches(0.7),
...
def slide_results(prs):
    ...
    for i, (label, value, detail, color) in enumerate(metrics):
        ...
        add_textbox(slide, left + Inches(0.1), Inches(1.7), Inches(2.0), Inches(0.6),
                    value, font_size=32, color=color, bold=True, alignment=PP_ALIGN.CENTER)
        add_textbox(slide, left + Inches(0.1), Inches(2.3), Inches(2.0), Inches(0.3),
                    label, font_size=13, color=DARK, bold=True, alignment=PP_ALIGN.CENTER)
        add_textbox(slide, left + Inches(0.1), Inches(2.65), Inches(2.0), Inches(0.3),
                    detail, font_size=10, color=MEDIUM_GRAY, alignment=PP_ALIGN.CENTER)

    # Staffing table header
    add_textbox(slide, Inches(0.8), Inches(3.6), Inches(11), Inches(0.4),
                "Average Daily Staffing by Shift", font_size=18, color=DARK, bold=True)
    ...
    for j, (header, w) in enumerate(zip(headers, col_widths)):
        add_textbox(slide, x_start + Inches(sum(col_widths[:j])), y,
    ...
    for i, row in enumerate(rows):
        ...
        add_shape(slide, x_start, y_row, Inches(sum(col_widths)), Inches(0.38), bg_color)
        for j, (val, w) in enumerate(zip(row, col_widths)):
            add_textbox(slide, x_start + Inches(sum(col_widths[:j])), y_row,
                        Inches(w), Inches(0.38),
                        val, font_size=12, color=DARK, alignment=PP_ALIGN.CENTER)


def slide_decisions(prs):
    """Slide 7: Design decisions & trade-offs."""
    slide = prs.slides.add_slide(prs.slide_layouts[6])
    make_slide_header(slide, "Design Decisions & Trade-offs",
    ...
    for i, (title, reasoning, badge) in enumerate(decisions):
        ...
        add_section_badge(slide, left + Inches(0.15), top + Inches(0.15), badge, DEEP_BLUE)
        add_textbox(slide, left + Inches(0.2), top + Inches(0.6), Inches(5.5), Inches(0.4),
                    title, font_size=16, color=DARK, bold=True)
        add_textbox(slide, left + Inches(0.2), top + Inches(1.0), Inches(5.5), Inches(1.2),
                    reasoning, font_size=12, color=DARK)


def slide_bonus_preferences(prs):
    """Slide 8: Bonus — Agent Preferences."""
    slide = prs.slides.add_slide(prs.slide_layouts[6])
    make_slide_header(slide, "Bonus: Agent Shift Preferences",
                     "Soft constraint integration for employee satisfaction")

    # Left: How it works
    add_textbox(slide, Inches(0.8), Inches(1.6), Inches(5.5), Inches(0.4),
                "How It Works", font_size=20, color=DARK, bold=True)
    ...
    add_bullet_list(slide, Inches(0.8), Inches(2.1), Inches(5.5), Inches(2.5),
                    pref_details, font_size=13)

    # Right: Results
    add_textbox(slide, Inches(7), Inches(1.6), Inches(5.5), Inches(0.4),
                "Results", font_size=20, color=DARK, bold=True)

    add_metric_card(slide, Inches(7), Inches(2.2), Inches(2.5), Inches(1.1),
                    "Satisfaction", "82%", value_color=SUCCESS)
    add_metric_card(slide, Inches(9.8), Inches(2.2), Inches(2.5), Inches(1.1),
                    "Preferred Shifts", "1,083/1,200", value_color=CYAN)

    add_textbox(slide, Inches(7), Inches(3.6), Inches(5.5), Inches(0.4),
                "Production Value", font_size=16, color=DARK, bold=True)
    ...
    add_bullet_list(slide, Inches(7), Inches(4.0), Inches(5.5), Inches(2.0),
                    prod_details, font_size=13)

    # Preference matrix placeholder
    add_shape(slide, Inches(0.8), Inches(4.8), Inches(5.5), Inches(2.0), LIGHT_GRAY, 0.03)
    add_textbox(slide, Inches(0.8), Inches(5.5), Inches(5.5), Inches(0.5),
...
def slide_bonus_fairness(prs):
    ...
    slide = prs.slides.add_slide(prs.slide_layouts[6])
    make_slide_header(slide, "Bonus: Fairness Metrics & Remediation",
                     "Quantitative equity measurement + actionable suggestions")

    # Left: Fairness
    add_textbox(slide, Inches(0.8), Inches(1.6), Inches(5.5), Inches(0.4),
                "Fairness Scoring", font_size=20, color=DARK, bold=True)
    ...
    add_bullet_list(slide, Inches(0.8), Inches(2.1), Inches(5.5), Inches(3.0),
                    fairness_details, font_size=13)

    # Right: Remediation
    add_textbox(slide, Inches(7), Inches(1.6), Inches(5.5), Inches(0.4),
                "Constraint Remediation", font_size=20, color=DARK, bold=True)
    ...
    add_bullet_list(slide, Inches(7), Inches(2.1), Inches(5.5), Inches(3.0),
                    rem_details, font_size=13)

    # Bottom: Why it matters
    add_shape(slide, Inches(0.6), Inches(5.4), Inches(12.0), Inches(1.2), CYAN, 0.03)
    add_textbox(slide, Inches(0.8), Inches(5.5), Inches(11.6), Inches(1.0),
...
def slide_production(prs):
    ...
    slide = prs.slides.add_slide(prs.slide_layouts[6])
    make_slide_header(slide, "Production Readiness",
    ...
    for i, (title, desc, icon) in enumerate(aspects):
        ...
        add_textbox(slide, left + Inches(0.2), top + Inches(0.1), Inches(0.5), Inches(0.4),
                    icon, font_size=20, color=DARK, alignment=PP_ALIGN.CENTER)
        add_textbox(slide, left + Inches(0.6), top + Inches(0.1), Inches(5.0), Inches(0.35),
                    title, font_size=15, color=DARK, bold=True)
        add_textbox(slide, left + Inches(0.6), top + Inches(0.45), Inches(5.0), Inches(0.85),
...
def slide_dashboard(prs):
    ...
    slide = prs.slides.add_slide(prs.slide_layouts[6])
    make_slide_header(slide, "Interactive Dashboard", "7 tabs: Forecast · Schedule · Summary · Constraints · Agents · Fairness · Preferences")

    # Screenshot placeholder - large
    add_shape(slide, Inches(0.8), Inches(1.5), Inches(11.7), Inches(5.0), LIGHT_GRAY, 0.02)
    add_textbox(slide, Inches(0.8), Inches(3.5), Inches(11.7), Inches(0.8),
    ...
    for i, feat in enumerate(features):
        ...
        add_textbox(slide, left, Inches(6.62), Inches(1.6), Inches(0.35),
                    feat, font_size=9, color=WHITE, bold=True, alignment=PP_ALIGN.CENTER)


def slide_challenges(prs):
    """Slide 12: Challenges & solutions."""
    slide = prs.slides.add_slide(prs.slide_layouts[6])
    make_slide_header(slide, "Challenges Solved", "Real engineering problems encountered and resolved")
    ...
    for i, (title, problem, solution, prob_color, sol_color) in enumerate(challenges):
        ...
        add_shape(slide, left, top, Inches(5.9), Inches(2.3), LIGHT_GRAY, 0.03)
        add_textbox(slide, left + Inches(0.2), top + Inches(0.1), Inches(5.5), Inches(0.35),
                    title, font_size=15, color=DARK, bold=True)
        add_textbox(slide, left + Inches(0.2), top + Inches(0.5), Inches(5.5), Inches(0.55),
                    f"Problem: {problem}", font_size=11, color=prob_color)
        add_textbox(slide, left + Inches(0.2), top + Inches(1.15), Inches(5.5), Inches(0.95),
                    f"Solution: {solution}", font_size=11, color=DARK)


def slide_next_steps(prs):
    """Slide 13: Future improvements."""
    slide = prs.slides.add_slide(prs.slide_layouts[6])
    make_slide_header(slide, "What's Next", "Production roadmap if this goes to production")
    ...
    for i, (title, color, items) in enumerate(categories):
        ...
        add_textbox(slide, left, Inches(1.62), Inches(3.8), Inches(0.45),
                    title, font_size=18, color=WHITE, bold=True, alignment=PP_ALIGN.CENTER)
        add_bullet_list(slide, left + Inches(0.2), Inches(2.3), Inches(3.4), Inches(3.5),
                        items, font_size=13)
...
def slide_thank_you(prs):
    ...
    add_textbox(slide, Inches(1), Inches(5.2), Inches(11), Inches(0.4),
